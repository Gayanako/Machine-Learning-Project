...
genai.configure(api_key="AIzaSyCjvggUnL0_rtBZsy3STKpwRxv_RP9X5mM")
...
def question_bp():
    if request.method == 'POST':
        ...
        return redirect(url_for('question_bp_diastolic'))
    progress = int((3 / TOTAL_QUESTIONS) * 100)
    return render_template('question.html', question="What is your blood pressure (Systolic) (mmHg)?", field_name="bp", field_type="number", progress_percentage=progress)

@app.route('/question/bp_sy', methods=['GET', 'POST'])
def question_bp_diastolic():
    if request.method == 'POST':
        return redirect(url_for('question_smoking'))
    progress = int((3 / TOTAL_QUESTIONS) * 100)
    return render_template('question.html', question="What is your blood pressure (Diastolic) (mmHg)?", field_name="bp", field_type="number", progress_percentage=progress)
...
def result():
    try:
        ...
        report = get_accuracy(variation_factor=3)
